...
> configured with PyPI trusted publishing for `modeldock`. The workflow already
> grants `contents: write` (job-level) so `gh release create` works. If trusted
> publishing is not configured, the PyPI publish step fails.
